...
export default function ChatWithSellerButton({ sellerId, listingId }: { sellerId: string; listingId: string }) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={busy}
      className="flex items-center justify-center gap-2 rounded-xl border border-rule-strong py-2.5 text-sm font-semibold transition-colors hover:border-brand-strong disabled:opacity-60"
    >
